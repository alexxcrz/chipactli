...
export function registrarRutasUtensilios(app, bdInventario) {
  app.get("/utensilios", (req, res) => {
    ...
    if (termino) {
      ...
      bdInventario.all(
        "SELECT * FROM utensilios ORDER BY COALESCE(NULLIF(TRIM(proveedor), ''), 'ZZZ'), nombre",
        (e, r) => {
  ...
  app.post("/utensilios/agregar", (req, res) => {
    const { codigo, nombre, proveedor, unidad, cantidad, costo } = req.body;
      ...
      if (!utensilio) {
        ...
        bdInventario.run(
          `INSERT INTO utensilios (codigo, nombre, proveedor, unidad, cantidad_total, costo_total, costo_por_unidad)
           VALUES (?,?,?,?,?,?,?)`,
          [codigo, nombre, String(proveedor || '').trim(), unidad, cantidad, costo, costoPorUnidad],
          function () {
        ...
        bdInventario.run(
          "UPDATE utensilios SET nombre=?, proveedor=?, unidad=?, cantidad_total=?, costo_total=?, costo_por_unidad=? WHERE id=?",
          [nombre, String(proveedor || utensilio.proveedor || '').trim(), unidad, nuevaCantidadTotal, nuevoCostoTotal, costoPorUnidad, utensilio.id],
          () => {
            bdInventario.run(
              "INSERT INTO historial_utensilios (id_utensilio, fecha_cambio, cambio_cantidad, cambio_costo) VALUES (?,?,?,?)",
  ...
  app.patch("/utensilios/:id", (req, res) => {
    const { nombre, proveedor, unidad, cantidad_total, costo_total } = req.body;
    ...
    bdInventario.get("SELECT * FROM utensilios WHERE id=?", [id], (e, utensilio) => {
      ...
      bdInventario.run(
        "UPDATE utensilios SET nombre=?, proveedor=?, unidad=?, cantidad_total=?, costo_total=?, costo_por_unidad=? WHERE id=?",
        [nombre, String(proveedor || '').trim(), unidad, nuevaCantidadTotal, nuevoCostoTotal, costoPorUnidad, id],
        () => {
  ...
  app.patch("/utensilios/:id/proveedor", (req, res) => {
    ...
    bdInventario.run("UPDATE utensilios SET proveedor=? WHERE id=?", [proveedor, id], function () {
